package day21collections;
           ...
           /*
  Collections are in 3 parts: 1)List(Interface):  a)ArrayList(Concrete Class)  b)LinkedList(Concrete Class)

 2)Queue(Interface): a)LinkedList(Concrete Class)  b)PriorityQueue(Concrete Class)  c)Deque(Interface)

  3)Set(Interface): a)HashSet(Concrete Class)  b)LinkedHashSet(Concrete Class)
   ...
   4)LinkedList has "node"s instead of elements.
...
public class C01LinkedList {
    ...
    // if you want to remove an integer "node", convert it to wrapper class then put it inside the remove()
    //parenthesis like the example below. If you put integer directly inside remove() parenthesis Java will accept it as index.

        linkedList.remove((Integer) 12); //typecasting
        System.out.println(linkedList);//[6, 5, 2]

        System.out.println(linkedList.remove()); //Retrieves and removes the first element of this list - Like "cut+paste" //6
