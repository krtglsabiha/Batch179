...
public class C02Lambda {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(checkWinterSeasonGivenAvg(courses, "Winter",96));
        printCourseNumOfStdHighest(courses);
        printCourseNumOfStdThirdHighest(courses);
    }

    //Example 1:Create a method to check if all average scores are greater than 9
    ...
    //Example 2:Create a method to check if at least on of the course name has "J" in it.
    ...
    //Example 3:Create a method to check if there is a season which is "Winter" and the averages score is less than 96
    public static boolean checkWinterSeasonGivenAvg(List<Course>list,String season,double avg){
    ...
    public static void printCourseNumOfStdHighest(List<Course>list){
        System.out.println(list.stream().sorted(Comparator.comparing(Course::getNumOfStd).reversed()).findFirst().get());//void olunca return kullanmiyoruz
    }
